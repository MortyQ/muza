...
On the pinned 1.8.2 the header is no longer needed — pass `FormData` as `data` and let the
library derive `multipart/form-data` (and `application/x-www-form-urlencoded` for
`URLSearchParams`) from the body itself:
...
Both existing uploads in this codebase still pass the header explicitly
(`ratingAndReviews/review/components/ProductReviewsTable.vue`,
`advertising/advertisingCampaign/components/CampaignsTable.vue`) — that was required on the
old 1.7.x pin and is still correct, so leave them alone. New uploads don't need it.
...
On those versions the header had to be set by hand:
...
When reading code written against the old pin, an explicit `multipart/form-data` on an upload
is that workaround, not a mistake — it is still correct on 1.8.2.
