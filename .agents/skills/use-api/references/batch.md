...
```ts
// Cache + select — the second generic is the raw response type before select
...
Not accepted — excluded by design, not oversights:
...
no longer holds up the fast ones on screen, so a `v-for` over `data` can render rows while
the rest are still in flight — check `status`, not `data !== null`.

`cache: { swr: true }` builds on that: cached items are published before the first network
round trip, then refreshed in the background. Note the app-wide policy in `app/main.ts`
already sets `cacheDefaults: { swr: true, ... }`, so a batch that opts into `cache: true`
inherits SWR from it — this is the behaviour you get by default here.
...
- `loading` — `true` only while some item has no data at all. An all-cached batch never
  flips it; bind spinners for the background refresh to `revalidating` instead.
...
- A failed revalidation is still a failure — `status: 'error'`, error in `errors` and
  ...
  `stale: true`, so the row shows the old value next to the error instead of going blank.
  Render on `status`/`stale`, not on `error === null`.
